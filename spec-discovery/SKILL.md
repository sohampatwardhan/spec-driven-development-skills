...
5. **Compare 2-3 viable approaches.** Lead with the recommendation. For each, state benefits, costs, risks, reversibility, and why it was accepted or rejected. Apply YAGNI and preserve existing project patterns. When an approach's viability, cost, or risk rests on a claim about a specific library, framework, SDK, API, CLI, or cloud service's current capability or behavior, use `context7-mcp` to resolve the library and affirmatively query that claim before writing it into the table — do not carry a capability claim from training-data memory into a decision a reviewer will rely on. A claim that fails or can't be verified this way is a cost/risk on that approach, not a fact to state as settled. This is lighter-weight than `spec-design`'s **Current Technology Evidence** section (discovery is comparing options, not committing to exact APIs), so a one-line note of what was checked and against which Context7 identity is enough; skip it entirely for approaches that don't depend on an evolving external technology.
