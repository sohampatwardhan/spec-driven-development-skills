...
- `00_state.json` — derived from `00_state.md`, written by `scripts/spec-check.py <spec-dir>` —
  emission is the default behavior of every invocation, `--emit-json` need not be passed
  explicitly. The one sidecar `--ready` gate-checking cares about; see the `Gate` note below.
- `04_tasks.json` — derived from `04_tasks.md`, written by the same default-emitting call.
- `05_execution.json` — derived from `05_execution.md`, written by the same default-emitting call.
- `02_requirements.json` — derived from `02_requirements.md`. Generated by `spec-requirements`'s
  own workflow rather than `spec-check.py` (it is parsed from EARS criteria, not from the task
  graph or timing ledger `spec-check.py` already computes) — the schema below is canonical
  regardless of which tool writes it.

Pass `--check-only` to validate without writing (a CI gate that must fail on a sidecar someone
forgot to regenerate, rather than silently repairing it). Pass `--emit-json` explicitly only when
a source doc that is not yet in canonical shape (e.g. `00_state.md` before its Gate table is
filled in) should make the check fail outright instead of merely warning — that is the one
difference between the default, best-effort emission and an explicit request.
