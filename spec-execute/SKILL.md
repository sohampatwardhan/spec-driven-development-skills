...
4. **Supervised PTY Event Loop**:
   ...
   - **Nested dispatch**: a worker that itself becomes a coordinator for sub-workers (a task's `--parent` hierarchy going more than one level deep) owns this same lifecycle for its own children — end each child's session and remove each child's worktree once integrated, before that worker reports its own `worker_done` upward. Do not let cleanup responsibility skip a level; a top-level sweep cannot see worktrees a nested worker never reported.
